    ...
    if unreadable:
        findings.append(Finding(
            ...
            recommendation=(
                ...
                "owns the path: on RHEL/cPanel, cphulkd ships under "
                "`/etc/logrotate.d/cphulkd` with its own retention, "
                "compression, and (where applicable) cPHulk service "
                "reopen directives — preserve those, and re-apply the "
                "ACL inside its existing `postrotate`/`lastaction` "
                "branch on both the live file and the `*.1`, `*.2.gz`, "
                "... copies. Do **not** add a second stanza for the "
                "same path — `logrotate` rejects duplicate entries "
                "with `error: duplicate log entry` and exit 1, and a "
                "second block would override the cPHulk-supplied "
                "rotation policy and reopen hooks. After editing, "
                "validate with `logrotate -d` to confirm there is no "
                "duplicate entry.\n"
                "\n"

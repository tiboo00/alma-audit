    ...
    if unreadable:
        findings.append(Finding(
            ...
            recommendation=(
                ...
                "tagged explicitly. For RHEL `rsyslog` / `logrotate.d`:\n"
                "\n"
                ...
                "/var/log/auth.log /var/log/auth.log.* 2>/dev/null "
                "|| true\n"
                "        endscript\n"
                "    }\n"
                "\n"
                ...
                "moment of creation (the standard logrotate hook); the "
                "`postrotate` re-applies the ACL to `*.1`, `*.2.gz`, etc. "
                "where `create` does not run.\n"
                "\n"
